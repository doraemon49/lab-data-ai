...
def run_train_eval_only_valid(model, loaders, tcfg, logger=None, keep_sparse=True):
    ...
    for epoch in range(tcfg.max_epoch):
        ...
        # 🔹GPU에서 손실 합을 누적(중간에 .item() 하지 않음)
        val_loss_sum = torch.zeros((), device=tcfg.device)

        # 🔹이진/다중 분기: torchmetrics AUROC (GPU)
        from torchmetrics.classification import BinaryAUROC, MulticlassAUROC
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            for bat in loaders["val"]:
                ...
                # batch_n = bat[0].shape[0]
                batch_n = len(bat[0])
                val_count += batch_n
                # 🔹GPU에서 누적
                val_loss_sum += loss.detach() * batch_n

                # 🔹AUROC 누적(프레임워크별 입력 형식에 주의)
                #  - BinaryAUROC: preds는 (N,) 확률 or 로짓, target은 (N,)
                #  - MulticlassAUROC: preds는 (N, C) 확률, target은 (N,)
                if n_classes == 2:
        ...
        curr_metric = val_auc  # 필요시 val_loss와 조합 가능

...
class ProtoCell(nn.Module):
    ...
    def forward(self, x, y, ct=None, sparse=True):
        ...
        if isinstance(x, list):
            x = torch.stack([
                torch.tensor(xi.toarray() if hasattr(xi, 'toarray') else xi)
                for xi in x
            ]).float().to(self.device)
        else:
            # numpy array 라면 바로 tensor 변환
            x = torch.tensor(x).to(self.device)
        y = y.to(self.device).long() # 정수형으로 명시적 변환
    ...
    def pretrain(self, x, y, ct=None, sparse=True):
        ...
        y = y.to(self.device) # type 에러 발생 시, .long()으로 변환
        if ct is not None:

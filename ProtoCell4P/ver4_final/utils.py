...
# utils.py (하단 아무 곳)
# 8→4→2→1 로 배치 축소 재시도를 하는 래퍼를 유틸에 둡니다.
import torch

def train_with_oom_retry(call, backoff=(768, 736, 512, 256, 128, 64, 32, 16, 8, 4, 2, 1), get_bs=None, set_bs=None):
    """
    call: 인자 없는 함수
    backoff: OOM 시 순차적으로 시도할 batch_size 후보
    get_bs/set_bs: 현재 batch_size get/set 콜백 (없으면 무시)
    """
    orig_bs = get_bs() if get_bs else None
    last_err = None
    try:
        try:
            return call()
        ...
        for bs in backoff:
            ...
            if get_bs and set_bs and orig_bs is not None:
                new_bs = min(orig_bs, bs)
                print(f"[OOM RETRY] batch_size {orig_bs} → {new_bs}")
                set_bs(new_bs)
            try:
                return call()
            except RuntimeError as e:

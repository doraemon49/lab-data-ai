...
class Config:
    ...
    def __init__(self, data="lupus", model="ProtoCell", split_ratio = [0.5, 0.25, 0.25], lr = 1e-4, max_epoch = 20,\
         ...
         lr_pretrain=1e-2, \
         # 새로 정의해준 인자들
         data_location=None, repeat=None, fold=None, lognorm=True):        
        # 새로 정의해준 인자들
        self.data = data
        self.task = task
        ...
        # (선택) 나중에 쓸 수 있으니 함께 저장
        self.exp_str = exp_str
        self.eval = eval
        self.subsample = subsample
        
        self._build()   # ← 여기서 split_datasets vs 기존 데이터 분기 처리
    ...
    def _build(self):
        ...
        if self.data.lower() == "split_datasets":
            ...
            X_tr, X_va, y_tr, y_va = train_test_split(
                Xtr, ytr, test_size=0.3, random_state=None, stratify=ytr  # ← 데이터 split에 seed 사용 안 함
            )

            # Dataloader에 맞게 reshape

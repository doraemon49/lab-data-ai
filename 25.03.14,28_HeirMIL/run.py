...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        for i in range(torch.cuda.device_count()):
            ...
            if mem_reserved < 100 * 1024 * 1024:  # 100MB 이하 사용 중이면 free로 간주
                free_gpus.append(i)
        
        if free_gpus:
            selected_gpu = free_gpus[0]
            args.device = f"cuda:{selected_gpu}"
            print(f"✅ Using GPU: cuda:{selected_gpu}")
        else:
            args.device = "cpu"
            print("⚠️ No free GPU available, using CPU.")
    else:
        args.device = "cpu"

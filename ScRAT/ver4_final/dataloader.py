...
def Custom_data_from_loaded(data, args):
    ...
    labels = data.obs['disease__ontology_label']  if 'disease__ontology_label' in data.obs else data.obs['disease_category'] # kidney는 'disease_category'안에 4개가 있음(원본). id_dict에서 묶어주니 ScRAT에서는 사용해도 괜찮다. (참고로 'label' 값을 사용하면 {0,1,2}로 매핑되어 있음.)

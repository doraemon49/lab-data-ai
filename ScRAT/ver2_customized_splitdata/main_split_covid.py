...
def train(x_train, x_valid, x_test, y_train, y_valid, y_test, id_train, id_test, data_augmented, data):
            ...
            with torch.no_grad():
                for batch in (valid_loader):
                    ...
                    if output_class == 1:
                        ...
                        loss_fn = nn.CrossEntropyLoss()
                        avg_logits = out.mean(dim=0, keepdim=True)  # (1, num_classes)
                        loss = loss_fn(avg_logits, y_.long().view(-1))
    ...
    try:
        if output_class == 1:
            ...
            test_auc = metrics.roc_auc_score(true, prob, multi_class='ovr', average='weighted')
    except ValueError:
        print("⚠️ AUC 계산 불가: test set에 한 클래스만 존재함")
        test_auc = np.nan
...
for repeat in range(args.repeat):
    ...
    for fold in range(args.n_splits):
        ...
        labels_ = np.concatenate([train_labels, test_labels]) # 틀린 곳 : 1
        ...
        test_ids = [test_patient_id[idx[0]] for idx in test_p_index] # 틀린 곳 : 2
        print(f"  → train 환자 ID: {train_ids}")
        print(f"  → test  환자 ID: {test_ids}")
        ...
        for idxs in test_p_index:
            idx = idxs[0]
            print(f"    ID: {test_patient_id[idx]}, Label: {test_labels[idx]}") # 틀린 곳 : 3
...
"""
main_split_cardio.py (main_split_covid.py에서 변경된 사항)
221~222 수정
"""
...
# dataloder.py 말고, dataloader_customized.py 로 실행함
from dataloader_customized import * 
...
for train_index, test_index in rkf.split(num):
    ...
    # 실제 환자 ID로 보기
    train_data_ids = [patient_id[p_idx[i][0]] for i in train_index]
    test_data_ids = [patient_id[p_idx[i][0]] for i in test_index]
    print(f"  → train 환자 ID: {train_data_ids}")
    print(f"  → test  환자 ID: {test_data_ids}")

        ...
        if ep % 1 == 0:
            ...
            with torch.no_grad():
                for batch in (valid_loader):
                    ...
                    # else:  # multi class 일 때, 검증 단계
                    #     # === Multi-class (예: 3-class) 검증 ===
                    #     # 1) 손실 계산 (셀 단위 CE 평균)
                    #     if out.dim() == 3:
                    #         B, N, C = out.shape
                    #         targets = y_.long().view(B, 1).expand(B, N)           # (B, N)
                    #         v_loss = nn.CrossEntropyLoss()(out.reshape(-1, C), targets.reshape(-1))
                    #         # 2) 확률/예측 산출
                    #         probs_cell = torch.softmax(out, dim=-1)               # (B, N, C)
                    #         # bag 확률: 셀 확률의 평균
                    #         probs_bag = probs_cell.mean(dim=1)                    # (B, C)
                    #         # majority voting: 셀 argmax의 최빈값
                    #         preds_cell = probs_cell.argmax(dim=-1)                # (B, N)
                    #         # (B,) — 각 배치의 최빈 클래스
                    #         preds_bag = []
                    #         for b in range(B):
                    #             vals = preds_cell[b].detach().cpu().numpy().astype(int).reshape(-1)
                    #             preds_bag.append(int(np.argmax(np.bincount(vals))))
                    #         preds_bag = np.array(preds_bag, dtype=int)
                    #         y_np = y_.detach().cpu().numpy().reshape(-1).astype(int)  # (B,)
                    #         # 기록
                    #         valid_loss.append(v_loss.item())
                    #         pred.extend(list(preds_bag))
                    #         true.extend(list(y_np))
                    #     elif out.dim() == 2:
                    #         N, C = out.shape
                    #         targets = y_.long().view(1).expand(N)                 # (N,)
                    #         v_loss = nn.CrossEntropyLoss()(out, targets)
                    #         # 확률/예측
                    #         probs_cell = torch.softmax(out, dim=-1)               # (N, C)
                    #         probs_bag = probs_cell.mean(dim=0)                    # (C,)
                    #         preds_cell = probs_cell.argmax(dim=-1)                # (N,)
                    #         voted = int(np.argmax(np.bincount(preds_cell.detach().cpu().numpy().astype(int))))
                    #         valid_loss.append(v_loss.item())
                    #         pred.append(voted)
                    #         true.append(int(y_.detach().cpu().numpy().reshape(-1)[0]))
                    #     else:
                    #         raise RuntimeError(f"Unexpected logits shape for multiclass: {out.shape}")

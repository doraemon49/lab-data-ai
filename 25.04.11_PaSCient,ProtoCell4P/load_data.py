...
#     # task 매개변수는 현재 구조상 있어야 하므로 받습니다.
#     adata = sc.read_h5ad("../data_ours/icb/icb_adata_scAce.h5ad")

#     # X: gene expression matrix
#     # 먼저 sparse/dense 변환 후
#     if not keep_sparse:
#         X = adata.X.toarray()
#     else:
#         X = adata.X

#     print(f"Loaded X shape: {X.shape}")  # Loaded X shape: (9292, 795)

#     # Y: label → classification을 위해 label을 사용 (예: outcome)
#     # 예시: 'label' 또는 'Combined_outcome' 컬럼
#     Y = adata.obs["label"].astype("category").cat.codes.values

#     print(f"Any NaN in X? {np.isnan(X).sum()}")
#     print(f"Any NaN in Y? {np.isnan(Y).sum()}")

#     # CT: cell type
#     CT = adata.obs["cell_type_annotation"].astype("category").cat.codes.values

#     # gene id → index
#     gene_idx = list(adata.var.index)

#     # cell type id → index
#     celltype_idx = adata.obs["cell_type_annotation"].astype("category").cat.categories.tolist()

#     return OurDataset(X, Y, CT, gene_idx, celltype_idx)

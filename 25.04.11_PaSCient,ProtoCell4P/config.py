...
class Config:
    ...
    def my_collate(self, batch):
        ...
        y = torch.tensor([item[1] for item in batch], dtype=torch.long)  # dtype 추가 # torch.LongTensor (int64) 이어야 합니다. 현재 y 가 torch.CharTensor (int8) 이라서 생긴 에러
        if len(batch[0]) == 3:
...
class Config_eval:
    ...
    def __init__(self, data, checkpoint_dir, checkpoint_name="best_model.pt", model="ProtoCell", \
        ...
        if data.lower() == "lupus":
            ...
            dataset = load_icb_data(task=task, load_ct=True)
        elif data.lower() == "covid_ours":
            dataset = load_covid_data(task=task, load_ct=load_ct, keep_sparse=self.keep_sparse)
        else:
            raise ValueError("Data [:s] not supported".format(data))
        ...
        self.train_set = list(zip(X_train, y_train))
        self.valid_set = list(zip(X_valid, y_valid))
        self.test_set = list(zip(X_test, y_test))

        self.train_set = list(zip(X_train, y_train, ct_train))
        self.valid_set = list(zip(X_valid, y_valid, ct_valid))
        self.test_set = list(zip(X_test, y_test, ct_test))
        self.device = device
        self.batch_size = batch_size
        self.h_dim = h_dim
        self.z_dim = z_dim
    ...
    def my_collate(self, batch):
        ...
        print(f"Shape of first x item: {x[0].shape}")  # 여기서 (795,) 나와야 정상!
        if len(batch[0]) == 3:
            ct = [item[2] for item in batch]
            return x, y, ct
        return x, y
